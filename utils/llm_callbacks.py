...
class AgentStateCallback(BaseCallbackHandler):
    ...
    def on_agent_finish(self, finish, **kwargs: Any) -> Any:
        ...
        if "total_tokens" in finish.return_values:
            ...
            self._print_colored(f"💰 Token消耗: {total_tokens} (估计成本: ${cost_estimate:.4f})", "green", True)
    ...
    def on_llm_end(self, response, **kwargs: Any) -> None:
        ...
        self._print_colored(f"✨ LLM 生成完成", "green")
                ...
                if generation:
                    for g_idx, g in enumerate(generation):
                        if hasattr(g, "message") and g.message:
                            self._print_colored(f"\n📝 模型输出 #{self.llm_call_count}.{g_idx}:", "cyan", True)
                            content = g.message.content if hasattr(g.message, "content") else str(g.message)
                            
                            # 尝试解析JSON格式的输出
                            try:
                                parsed_content = json.loads(content)
                                self._print_colored(parsed_content, "cyan")
                            except:
